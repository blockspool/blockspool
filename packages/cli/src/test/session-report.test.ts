...
describe('generateSessionReport', () => {
  it('produces valid markdown with all sections', () => {
    ...
    expect(report).toContain('# BlockSpool Session Report');
    ...
    expect(report).toContain('Generated by [BlockSpool]');
  });
...
describe('writeSessionReport', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'blockspool-report-test-'));
  ...
  it('creates file in .blockspool/reports/', () => {
    const report = '# Test Report\nHello';
    const relPath = writeSessionReport(tmpDir, report);

    expect(relPath).toMatch(/^\.blockspool\/reports\/session-.*\.md$/);
  ...
  it('creates reports directory if it does not exist', () => {
    const reportsDir = path.join(tmpDir, '.blockspool', 'reports');
