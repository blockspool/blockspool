 ...
 * of a BlockSpool session that can be pasted into Slack, PRs, etc.
 */
...
export function generateSessionReport(ctx: SessionReportContext): string {
  ...
  lines.push('# BlockSpool Session Report');
  lines.push('');
  ...
  // Footer
  lines.push('---');
  lines.push(`*Generated by [BlockSpool](https://github.com/blockspool/blockspool) v${CLI_VERSION}*`);

  return lines.join('\n');
}

/**
 * Write a session report to `.blockspool/reports/session-<ISO-date>.md`.
