...
export function buildScoutPrompt(
  ...
  const parts = [
    ...
    '',
    'STEP 4 — Check AST Findings: If the Codebase Structure section above includes "AST Findings", review them first. HIGH severity findings (security, correctness) should become proposals. MEDIUM findings are good candidates if they have real impact. LOW findings are FYI only.',
    '',
    'STEP 5 — Propose: Only after reading source files, write proposals with specific file paths and line-level detail.',
    '',
  ...
  parts.push(
    ...
    '- `rationale` (string): why this improvement matters',
    '- `estimated_complexity` (string): "trivial", "simple", "moderate", or "complex"',
    ...
    '',
    'Optional:',
    '- `target_symbols` (string[]): function/class/variable names this change modifies (e.g. ["handleLogin", "AuthService"]). Helps detect when two proposals can safely edit the same file in parallel.',
    '- `severity` (string): "blocking", "degrading", "polish", or "speculative". Defaults to "polish" if omitted. Use "blocking" for security/correctness issues, "degrading" for bugs that degrade functionality, "speculative" for exploratory refactors.',
    '',
    '## Scoring',
    '',
    'Proposals are ranked by `impact_score × confidence × severity_weight`. Severity weights: blocking=3×, degrading=2×, polish=1×, speculative=0.5×. This means a subtle security fix (blocking) outranks a flashy refactor (speculative).',
    '',
    '## Self-Review (Adversarial)',
    '',
    'After generating your proposals, critically review EACH one as if you were a skeptical senior engineer:',
    '- Is the impact_score realistic, or inflated to pass the filter?',
    '- Is the confidence warranted by what you actually read, or are you guessing?',
    '- Could the fix introduce regressions? Is the scope underestimated?',
    '- Is this a real improvement or just busywork?',
    '',
    'Adjust `confidence` and `impact_score` DOWN if your review finds problems. Add a `review_note` field (string) to each proposal explaining your assessment.',
    'If a proposal is fundamentally flawed, set confidence to 0 (it will be filtered out).',
    '',
    '## Output',
    '',
...
// Delimiter includes "_" so it cannot appear in base64 payload lines.
const PR_BODY_B64_DELIMITER = 'PROMPTWHEEL_PR_BODY_B64_EOF';

function buildInlinePrBody(ticket: { title: string; description: string | null }): string {
  return `${ticket.description?.slice(0, 500) ?? ticket.title}\n\nGenerated by PromptWheel`;
}

function encodeForShellBase64Heredoc(input: string): string {
...
export function buildInlineTicketPrompt(
  ...
  const inlinePrBodyBase64 = encodeForShellBase64Heredoc(buildInlinePrBody(ticket));
  ...
  return [
    ...
    '',
    ...(createPrs ? [
      'If rebase fails with conflicts, abort (`git rebase --abort`) and report STATUS: failed with reason "merge conflict during rebase".',
      '',
    ] : []),
    ...(createPrs ? [
      '## Step 5 — Create PR',
      ...
      '```bash',
      `cd ${worktree}`,
      'PR_BODY_FILE=.promptwheel-pr-body.md',
      `cat <<'${PR_BODY_B64_DELIMITER}' | node -e "let input = ''; process.stdin.setEncoding('utf8'); process.stdin.on('data', chunk => { input += chunk; }); process.stdin.on('end', () => { require('node:fs').writeFileSync(process.argv[1], Buffer.from(input.replace(/\\s+/g, ''), 'base64').toString('utf8')); });" "$PR_BODY_FILE"`,
      inlinePrBodyBase64,
      PR_BODY_B64_DELIMITER,
      `gh pr create --title "${shellEscape(ticket.title)}"${draft ? ' --draft' : ''} --body-file "$PR_BODY_FILE"`,
      '```',
      '',
    ] : []),
    '## Output',
